...
from mmuav_msgs.cfg import MavXYCtlParamsConfig
...
class HorizontalControl:
    '''
    Class implements ROS node for cascade (x, vx, y, vy) PID control for MAV horizontal position.
    Subscribes to:
    ...
    def __init__(self):
        ...
        #########################################################
        #########################################################
        # Parameters for roll and pitch are the same because of UAV symmetry
        kp_xy = 0.8
        ki_xy = 0
        kd_xy = 0
        ...
        self.pid_x.set_kp(kp_xy)
        self.pid_x.set_ki(ki_xy)
        self.pid_x.set_kd(kd_xy)

        self.pid_x.set_lim_high(pid_xy_limit)  # max vx speed
        self.pid_x.set_lim_low(-pid_xy_limit)  # min vx speed

        # Add parameters for vx controller
        self.pid_vx.set_kp(kp_v_xy)
        self.pid_vx.set_ki(ki_v_xy)
        self.pid_vx.set_kd(kd_v_xy)

        self.pid_vx.set_lim_high(pid_vxy_limit)     # max wing angle
        self.pid_vx.set_lim_low(-pid_vxy_limit)     # min wing angle

        # Add parameters for y controller
        self.pid_y.set_kp(kp_xy)
        self.pid_y.set_ki(ki_xy)
        self.pid_y.set_kd(kd_xy)

        self.pid_y.set_lim_high(pid_xy_limit)  # max vy speed
        self.pid_y.set_lim_low(-pid_xy_limit)  # min vy speed

        # Add parameters for vy controller
        self.pid_vy.set_kp(kp_v_xy)
        self.pid_vy.set_ki(ki_v_xy)
        self.pid_vy.set_kd(kd_v_xy)

        self.pid_vy.set_lim_high(pid_vxy_limit)     # max wing angle
        self.pid_vy.set_lim_low(-pid_vxy_limit)     # min wing angle
        ...
        self.angle_control_flap_command_pub_0 = rospy.Publisher('angle_wing_0_ref_value', Float32,
                                                                queue_size=1)
        self.angle_control_flap_command_pub_1 = rospy.Publisher('angle_wing_1_ref_value', Float32,
                                                                queue_size=1)
        self.angle_control_flap_command_pub_2 = rospy.Publisher('angle_wing_2_ref_value', Float32,
                                                                queue_size=1)
        self.angle_control_flap_command_pub_3 = rospy.Publisher('angle_wing_3_ref_value', Float32,
                                                                queue_size=1)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # print "\n"
            angle_control_flap_x_tf = math.cos(self.yaw_mv) * angle_control_flap_x + math.sin(
                self.yaw_mv) * angle_control_flap_y;
            angle_control_flap_y_tf = -math.sin(self.yaw_mv) * angle_control_flap_x + math.cos(
                self.yaw_mv) * angle_control_flap_y;

            ########################################################
            ########################################################
            angle_control_flap_command_pub_0_msg = Float32()
            angle_control_flap_command_pub_0_msg = angle_control_flap_x_tf
            angle_control_flap_command_pub_1_msg = Float32()
            angle_control_flap_command_pub_1_msg = angle_control_flap_y_tf
            angle_control_flap_command_pub_2_msg = Float32()
            angle_control_flap_command_pub_2_msg = angle_control_flap_x_tf
            angle_control_flap_command_pub_3_msg = Float32()
            angle_control_flap_command_pub_3_msg = angle_control_flap_y_tf
