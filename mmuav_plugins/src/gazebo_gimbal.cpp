...
// }
  ...
  void GazeboGimbal::Update() {
    ...
    // ignition::math::Pose3d pose_gimbal = world->GetLink("gimbal_link_4")->WorldPose();
    // ROS_INFO("-------NORMAL-----------");
    // ROS_INFO("Pose %f %f %f", pose.Pos().X(), pose.Pos().Y(),pose.Pos().Z());
    // ROS_INFO("Orientation %f %f %f %f", pose.Rot().X(), pose.Rot().Y(),pose.Rot().Z(), pose.Rot().W());
    double x, y, z, w;
    x = pose.Rot().X();
    y = pose.Rot().Y();
    z = pose.Rot().Z();
    w = pose.Rot().W();
    // ROS_INFO("Euler base_link %f %f %f", pose.Rot().Euler().X(), pose.Rot().Euler().Y(),pose.Rot().Euler().Z());
    // ROS_INFO("Euler gimbal %f %f %f", pose_gimbal.Rot().Euler().X(), pose_gimbal.Rot().Euler().Y(),pose_gimbal.Rot().Euler().Z());
